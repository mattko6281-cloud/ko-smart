...
//  메타수학용 프롬프트 가이드 원문
// ─────────────────────────────────────────────────────────────
const META_PROMPT_GUIDE = `[TikZ 수학 그래프 렌더링 엄격한 스타일 가이드: 평가원(KICE) 스타일 v2.1 - 찐최종 20260603]
...
2. [전역 환경 및 스마트 캔버스 배율 (일러스트레이터 대지 맞춤 방식)]
- 폰트/선 두께 고정: HWP 12.5% 삽입 시 가독성을 위해 폰트 스케일(scale=2.2)과 선 두께는 절대 변경하지 마세요. (이는 텍스트를 고정 크기로 덧입히는 것과 같습니다.)
- 대지 맞춤형 x, y 동적 계산 (핵심): 일러스트레이터에서 일정한 크기의 대지에 맞춰 도형을 꽉 차게 늘리거나 줄이는 것과 완벽히 동일한 원리입니다. 렌더링될 그래프의 '가로와 세로 중 긴 쪽'의 물리적 길이가 항상 약 10cm (TikZ 기준)가 되도록 좌표 구간을 계산하여 x와 y 배율을 동적으로 설정하세요.
- 예시: x축 표현 구간이 폭 5라면 x=2.0cm, 폭 10이라면 x=1.0cm로 설정.
...
7. [기하학적 기호 및 보조선 표시]
- 직각 기호: 수직(직각) 기호는 scope의 rotate 각도를 조절하여 반드시 도형 안쪽으로 그려지도록 세팅합니다.
- 길이 표시 (평가원 점선 스타일): 변 바깥쪽에 길이를 표시할 때는 직선 대신 부드럽게 휘어지는 점선인 베지어 곡선(.. controls (x,y) ..)이나 \\draw[dashed] (A) to[bend right=25] (B);를 사용합니다. 이때 텍스트는 선 바깥으로 밀어내지 말고, 노드에 [fill=white, inner sep=3pt] 옵션을 주어 점선 한가운데에 위치시켜 선이 자연스럽게 끊긴 것처럼 보이게 연출하세요. (단, 하얀 배경이 주변 실선 도형을 침범하지 않도록 주의합니다.)
