...
//  메타수학용 프롬프트 가이드 원문
// ─────────────────────────────────────────────────────────────
const META_PROMPT_GUIDE = String.raw`[INF_TikZ 수학 그래프 렌더링 엄격한 스타일 가이드: KO-SMART 최적화 최종본_260806]
...
1. [KO-SMART 웹 렌더링 최적화 - 절대 규칙]
- 문서 구조 자동화 대응: KO-SMART 시스템이 렌더링 전처리를 통해 여백 설정(\documentclass[tikz, border=2pt]{standalone})과 document 환경을 자동으로 래핑하므로, 전체 문서 형태가 아닌 오직 \begin{tikzpicture} ... \end{tikzpicture} 내부 코드만 출력하세요.
- 한글 텍스트 전면 허용: 시스템이 자체적으로 한글 패치(\usepackage{kotex})를 주입하므로, 노드(Node)나 텍스트 출력 부분에 자유롭게 한글을 포함하여 작성하십시오. 단, 코드 내부에 패키지를 직접 선언할 필요는 없습니다.
...
- 레이어(Layer) 렌더링 순서 엄수: 도형의 색칠(fill)이나 음영 영역은 무조건 $x$, $y$축이나 메인 그래프보다 먼저 코드 상단에 작성하여 가장 밑바탕(배경)에 깔리도록 하세요. (순서: 1. 음영/색칠 ➔ 2. 축 ➔ 3. 메인 그래프 ➔ 4. 점과 라벨)
...
- 절연 기법 및 넉넉한 휨 각도: 텍스트 노드에 [midway, fill=white, inner sep=1pt] 옵션을 주어 점선 한가운데를 끊고 들어가게 연출하되, 이 하얀색 배경 박스가 메인 실선을 지우개처럼 파먹는 일이 절대 없도록 bend 각도를 최소 25~35 이상으로 넉넉하게 부여하여 실선에서 완전히 이격시키세요. (선분이 짧을수록 bend 값을 더 키워야 합니다.)

[필수 코드 예시] 길이 표시 점선을 그릴 때는 반드시 아래 코드를 그대로 복붙해서 응용하세요.
\draw[dashed] (A) to[bend left=30] node[midway, fill=white, inner sep=1pt] {수식} (B);
