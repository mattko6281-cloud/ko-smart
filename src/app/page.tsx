...
//  메타수학용 프롬프트 가이드 원문
// ─────────────────────────────────────────────────────────────
const META_PROMPT_GUIDE = `[TikZ 수학 그래프 렌더링 엄격한 스타일 가이드: 평가원(KICE) 스타일 v2.1 - 최종 20260603]
...
- 길이 표시: 변 바깥쪽에 길이를 표시할 때는 직선 대신 부드럽게 휘어지는 점선인 베지어 곡선(.. controls (x,y) ..)이나 \\draw[dashed] (A) to[bend right=25] node[midway] {$12$} (B); 를 사용합니다.
